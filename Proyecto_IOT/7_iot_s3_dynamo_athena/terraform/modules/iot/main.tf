...
# Regla 3: Temperatura > 30 → Lambda Alerta

# Regla 3: Temperatura > 30 → Lambda Alerta
resource "aws_iot_topic_rule" "alert_rule" {
  name        = "SensorAlertTemperature_${var.environment}"
  description = "Dispara alerta cuando temperatura supera 30 grados"
  enabled     = true
...
# Regla 4: Guardar cada evento en tabla de recientes
resource "aws_iot_topic_rule" "dynamodb_events_rule" {
  name        = "SensorEventsToRecent_${var.environment}"
  description = "Guarda cada evento en tabla de recientes con timestamp"
  enabled     = true
  sql         = "SELECT * FROM 'lab/sensors/data'"
  sql_version = "2016-03-23"
  ...
  dynamodbv2 {
    role_arn = var.lab_role_arn
    put_item {
      table_name = var.sensor_events_table_name
    }
  }
}
